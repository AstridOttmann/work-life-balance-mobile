...
function elapsedMs(t: TrackerState): number {
  const running =
    t.status === "running" && t.startTimestamp != null
      ? Date.now() - t.startTimestamp
      : 0;
    ...
    async (type: "WORK" | "FREE") => {
      ...
      if (tracker.status === "running") {
        if (tracker.blockId && tracker.dailyEntryId && tracker.startTime) {
          const newAcc =
            tracker.accumulatedMs +
            (tracker.startTimestamp != null
              ? Date.now() - tracker.startTimestamp
              : 0);
        ...
        const result = await timeBlocksApi.update(tracker.blockId, {
          ...
          startTime: tracker.startTime + ":00",
          paused: false,
          elapsedMs: tracker.accumulatedMs,
          segmentStartTime: now.format("HH:mm:ss"),
        });
        if (result === API_ERROR) {
          toast.error("Failed to resume tracker");
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.summaryCard}>
        <View style={styles.summaryItem}>
          ...
          <Text
            variant="titleMedium"
            style={[styles.summaryValue, { color: PRIMARY }]}
          >
            {formatHM(summary.work)}
          </Text>
        </View>
        ...
        <View style={styles.summaryItem}>
          ...
          <Text
            variant="titleMedium"
            style={[styles.summaryValue, { color: SECONDARY }]}
          >
            {formatHM(summary.free)}
          </Text>
        </View>
      </View>
...
function TrackerCard({
  ...
  const pauseDisabled =
    tracker.status === "idle" ||
    (tracker.status === "paused" && otherStatus === "running");
    ...
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        <MaterialCommunityIcons
          name={isWork ? "briefcase" : "spa"}
          size={20}
          color={accentColor}
        />
        <Text variant="titleMedium" style={styles.cardTitle}>
          {isWork ? "Work Time" : "Free Time"}
        </Text>
        {tracker.status !== "idle" && (
        ...
        <View style={styles.ringContainer}>
          ...
          <View style={styles.timerOverlay}>
            <Text
              style={[
                styles.timerText,
                { color: tracker.status === "idle" ? "#bbb" : "#1A1A1A" },
              ]}
            >
        ...
        <View style={styles.buttons}>
          <View style={styles.btnCol}>
            <TouchableOpacity
              ...
              style={[
                styles.btn,
                { backgroundColor: startDisabled ? "#e0e0e0" : PRIMARY },
              ]}
            >
              <MaterialCommunityIcons
                name="play"
                size={24}
                color={startDisabled ? "#9e9e9e" : "#fff"}
              />
            </TouchableOpacity>
          ...
          <View style={styles.btnCol}>
            <TouchableOpacity
              ...
              style={[
                ...
                styles.btnOutlined,
                {
                  borderColor: pauseDisabled ? "#e0e0e0" : "#1A1A1A",
          ...
          <View style={styles.btnCol}>
            <TouchableOpacity
              ...
              style={[
                ...
                styles.btnOutlined,
                {
                  borderColor: stopDisabled ? "#e0e0e0" : "#ba1a1a",
...
const styles = StyleSheet.create({
  ...
  btnCol: { alignItems: "center", gap: 4, paddingLeft: 4, paddingBottom: 4 },
  btn: {
    width: 56,
    height: 56,
    borderRadius: 12,
